...
#
# Commands:
#   *`bat weather`* - Get the weather for Insight Headquarters
#   *`bat weather <location>`* - Get the weather for <location>
#
# Notes:
    ...
    msg.http(googleurl)
      ...
      .get() (err, res, body) ->
        ...
        if result.results.length > 0
          ...
          darkSkyMe msg, lat,lng , options.separator, (darkSkyText) ->
            response = "Weather for #{result.results[0].formatted_address}#{options.separator}#{darkSkyText}"
                .replace /(\d+\.?\d*)°C/g, (match) ->
                    ...
                    match = Math.round(centigrade * (9/5) + parseInt(32, 10)) + "°Freedom Units"
            # response = "Weather for #{result.results[0].formatted_address}#{options.separator}#{darkSkyText}"
            #   .replace /(\d+\.?\d*)°C/g, (match) ->
            #     centigrade = match.replace /°C/, ''
          ...
          msg.send "Couldnot find #{location}"
